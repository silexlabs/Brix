...
import slplayer.core.Builder;
import slplayer.util.MacroTools;
	...
	static private function discoverGroupableChilds( elt:HtmlDom ) : List<HtmlDom>
	{
		...
		for (childCnt in 0...directChilds.length)
		{
			...
			for ( classAttr in childElt.getAttribute("class").split(" ") )
			{
				...
				if ( !MacroTools.is( switch( Context.getType(potentialClasses.first()) ) { case TInst( classRef , params ): classRef.get(); default : } , "slplayer.ui.group.IGroupable" ) )
					continue;
