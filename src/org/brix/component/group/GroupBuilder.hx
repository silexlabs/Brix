 ...
 */
package org.brix.component.group;

import org.brix.core.Builder;
import org.brix.util.MacroTools;
		...
		for (childCnt in 0...directChilds.length)
		{
			...
			if ( childElt.getAttribute("class") != null ){
				for ( classAttr in childElt.getAttribute("class").split(" ") )
				{
					...
					if ( !MacroTools.is( switch( Context.getType(potentialClasses.first()) ) { case TInst( classRef , params ): classRef.get(); default : } , "org.brix.component.group.IGroupable" ) )
						continue;
