 ...
 */
package org.brix.component.navigation;

import js.Lib;
import js.Dom;

import org.brix.component.ui.DisplayObject;
import org.brix.component.navigation.link.LinkBase;
import org.brix.component.navigation.transition.TransitionData;
...
class Page extends DisplayObject, implements IGroupable
{
	...
	{// trace("closeOthers("+transitionData+") - "+preventTransitions);
		...
		for (idxPageNode in 0...nodes.length)
		{
			...
			var pageInstances:List<Page> = getBrix().getAssociatedComponents(pageNode, Page);
	...
	{// trace("doOpen "+transitionData+", "+name+" - "+preventTransitions);
		...
		for (idxLayerNode in 0...nodes.length)
		{
			...
			var layerInstances:List<Layer> = getBrix().getAssociatedComponents(layerNode, Layer);
